...
mod tests {
  ...
  fn create_init_mint_ix(payer: Pubkey, program_id: Pubkey) -> (Pubkey, Pubkey, Instruction) {
    let (mint, bump_seed) = Pubkey::find_program_address(&[b"token_mint"], &program_id);
    let (mint_auth, bump_seed) = Pubkey::find_program_address(&[b"token_auth"], &program_id);
    ...
    let init_mint_ix = Instruction {
        ...
        accounts: vec![
            ...
            AccountMeta::new_readonly(SYSTEM_RENT_ID, false)
        ],
        data: vec![3],,,,,,,,,,,,;
    ...
    async fn test_initialize_mint_instruction() {
        ...
        let (_mint, _mint_auth, init_mint_ix) = crate_init_mint_ix(payer.pubkey(), program_id);
